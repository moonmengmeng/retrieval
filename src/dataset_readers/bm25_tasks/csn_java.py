...
def get_question(entry):
    ...
    return csn_javaBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return csn_javaBM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return csn_javaBM25Task.norm(entry['target'])


class csn_javaBM25Task:
    name = 'csn_java'
