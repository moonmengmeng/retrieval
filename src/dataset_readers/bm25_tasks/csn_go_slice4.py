...
def get_question(entry):
    ...
    return csn_go_slice4BM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return csn_go_slice4BM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return csn_go_slice4BM25Task.norm(entry['target'])


class csn_go_slice4BM25Task:
    name = 'csn_go_slice4'
    ...
    def __init__(self, dataset_split, setup_type, ds_size=None):
        ...
        dataset = load_dataset("mengmengmmm/csn_go_trainuse_slice4")
        print(dataset)
