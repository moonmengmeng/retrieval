...
def get_question(entry):
    ...
    return java2csBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return java2csBM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return java2csBM25Task.norm(entry['target'])


class java2csBM25Task:
    name = 'java2cs'
