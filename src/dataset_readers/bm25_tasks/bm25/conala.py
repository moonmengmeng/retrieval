...
def get_question(entry):
    ...
    return conalaBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return conalaBM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return conalaBM25Task.norm(entry['target'])


class conalaBM25Task:
    name = 'conala'
