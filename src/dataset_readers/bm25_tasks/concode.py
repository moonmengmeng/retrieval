...
def get_question(entry):
    ...
    return concodeBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return concodeBM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return concodeBM25Task.norm(entry['target'])


class concodeBM25Task:
    name = 'concode'
