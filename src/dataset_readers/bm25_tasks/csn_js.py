...
def get_question(entry):
    ...
    return csn_jsBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return csn_jsBM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return csn_jsBM25Task.norm(entry['target'])


class csn_jsBM25Task:
    name = 'csn_js'
