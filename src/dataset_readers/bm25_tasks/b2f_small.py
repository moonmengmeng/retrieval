...
def get_question(entry):
    ...
    return b2f_smallBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return b2f_smallBM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return b2f_smallBM25Task.norm(entry['target'])


class b2f_smallBM25Task:
    name = 'b2f_small'
    ...
    def __init__(self, dataset_split, setup_type, ds_size=None):
        ...
        dataset = load_dataset("mengmengmmm/B2F_smal")
        print(dataset)
